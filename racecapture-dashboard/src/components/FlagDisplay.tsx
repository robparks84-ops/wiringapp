...
// Flagtronics broadcasts flag status over CAN bus. RaceCapture reads it as a channel.
// Common channel names from Flagtronics FT200 CAN mapping:
...
// Flagtronics CAN value mapping (from FT200 documentation)
...
export default function FlagDisplay() {
  ...
  useEffect(() => {
    // Find the flag channel — try known names, then scan for any channel with "flag" in the name
    ...
    if (flag !== currentFlag) {
      ...
      // Play a beep on flag change (except unknown)
      if (flag !== 'unknown') {
        try {
